...
# -----------------------------------------------------------------------------
user_data = {}  # структура описана далее
...
def cmd_start(message):
    ...
    bot.send_message(chat_id, t(chat_id, "choose_language"),
...
def handle_category(call):
    ...
    # Удаляем любую Reply-клавиатуру (если была) перед отправкой inline-кнопок
...
def handle_edit_item_request(call):
    ...
    bot.send_message(chat_id, f"Current item: {cat} — {flavor} — {price}₺ (in cart {old_qty} pcs).\n{t(chat_id, 'enter_new_qty')}",
                     reply_markup=types.ReplyKeyboardRemove())
...
def handle_finish_order(call):
    ...
    if user_points > 0:
        ...
        bot.send_message(chat_id, types.ReplyKeyboardRemove())
        bot.send_message(
            chat_id,
            f"🛒 Ваш заказ:\n"
            + "\n".join(f"{item['category']}: {item['flavor']} — {item['price']}₺" for item in cart)
...
def universal_handler(message):
    ...
    if data.get('edit_phase'):
        ...
        if phase == 'enter_category_picture_url':
            ...
            if cat0 and new_url:
                ...
                bot.send_message(chat_id, "Invalid URL. Try again or press Cancel.", reply_markup=edit_action_keyboard())
    ...
    if text == f"🗑️ {t(chat_id, 'clear_cart')}":
        ...
        data["wait_for_comment"] = False
        # Удаляем Reply-клавиатуру и показываем главное меню
        bot.send_message(chat_id, t(chat_id, "cart_cleared"), reply_markup=types.ReplyKeyboardRemove())
        bot.send_message(chat_id, t(chat_id, "choose_category"), reply_markup=get_inline_main_menu(chat_id))
        user_data[chat_id] = data
        return
    ...
    if text == "/stats":
        ...
        # Удаляем Reply-клавиатуру перед отправкой отчета
        bot.send_message(chat_id, types.ReplyKeyboardRemove())
