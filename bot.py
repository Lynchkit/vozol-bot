...
# ------------------------------------------------------------------------
#   12. Inline-клавиатура комментария: Оформить заказ / Назад
#   (заменяет прежнюю reply-клавиатуру comment_keyboard)
# ------------------------------------------------------------------------
def comment_keyboard(chat_id: int) -> types.InlineKeyboardMarkup:
    """
    Inline-кнопки: Оформить заказ и Назад.
    Используем Inline, чтобы клавиатура не была reply и оставалась видимой
    после отправки сообщений ("Комментарий сохранён" и т.д.).
    """
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        types.InlineKeyboardButton(text=t(chat_id, "send_order"), callback_data="send_order_inline"),
        types.InlineKeyboardButton(text=t(chat_id, "back"), callback_data="back_from_comment")
    )
    return kb
...
# ------------------------------------------------------------------------
#   13b. Общая функция для обработки подтверждения заказа (вынесена)
# ------------------------------------------------------------------------
def process_send_order(chat_id: int, message=None):
    """
    Вынесенная логика оформления заказа — используется и для inline-кнопки,
    и для существующей Reply-кнопки (если где-то ещё используется).
    message может быть исходным Message или call.message (при callback).
    """
    ...
    # Баллы
    pts_spent = data.get("pending_points_spent", 0)
    ...
    if pts_earned > 0:
        cursor_local.execute("UPDATE users SET points = points + ? WHERE chat_id = ?", (pts_earned, chat_id))
        conn_local.commit()
        bot.send_message(chat_id, f"👍 Вы получили {pts_earned} бонусных баллов за этот заказ.")

    # Реферальная премия (если была)
    ...
    # Уведомления (используем display name)
    try:
        user_chat = bot.get_chat(chat_id)
        display = f"@{user_chat.username}" if getattr(user_chat, "username", None) else (user_chat.first_name or str(chat_id))
    except Exception:
        display = f"user_{chat_id}"
    ...
    # Inline-кнопки для админской зоны (Отменить / Доставлено)
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        types.InlineKeyboardButton(text="❌ Отменить заказ", callback_data=f"cancel_order|{order_id}"),
    ...
    # Пользователю — подтверждение и история
    bot.send_message(chat_id, t(chat_id, "order_accepted"),
...
# ----- Callback: Оформить заказ через inline-кнопку -----
@bot.callback_query_handler(func=lambda c: c.data == "send_order_inline")
def callback_send_order_inline(call: types.CallbackQuery):
    bot.answer_callback_query(call.id)
    # убираем inline-кнопки у исходного сообщения (чтобы нельзя было нажать дважды)
    try:
        bot.edit_message_reply_markup(chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=None)
    except Exception:
        pass
    process_send_order(call.from_user.id, call.message)

# ----- Callback: Назад из этапа комментария -----
@bot.callback_query_handler(func=lambda c: c.data == "back_from_comment")
def callback_back_from_comment(call: types.CallbackQuery):
    bot.answer_callback_query(call.id)
    chat_id = call.from_user.id
    data = user_data.get(chat_id, {})
    # Возвращаемся на шаг ввода контакта
    data['wait_for_comment'] = False
    data['wait_for_contact'] = True
    user_data[chat_id] = data

    # убираем inline-кнопки у исходного сообщения
    try:
        bot.edit_message_reply_markup(chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=None)
    except Exception:
        pass
...
# ------------------------------------------------------------------------
#   14. Клавиатура редактирования меню (/change) — ВСЁ НА АНГЛИЙСКОМ
# ------------------------------------------------------------------------
# (оставлено прежним порядком нумерации в файле; далее идут остальные хендлеры)
# ------------------------------------------------------------------------
...
def handle_finish_order(call):
    ...
    if user_points > 0:
        ...
        # Перейдём сразу к вводу адреса/контакта/комментария; у нас отображается inline-клавиатура с "Оформить" и "Назад"
...
def handle_contact_input(message):
    ...
    kb = comment_keyboard(chat_id)
    # Показываем inline-кнопки: Оформить / Назад
...
def handle_comment_input(message):
    ...
    if message.content_type == 'text' and text != t(chat_id, "send_order"):
        ...
        # Комментарий сохранён — показываем inline-кнопки снова (Оформить / Назад)
        bot.send_message(chat_id, t(chat_id, "comment_saved"), reply_markup=comment_keyboard(chat_id))
        user_data[chat_id] = data
        return

    # Пользователь подтвердил отправку заказа (через текст reply-кнопки)
    if text == t(chat_id, "send_order"):
        # Поддерживаем старый путь, если кто-то использует reply-кнопку
        process_send_order(chat_id, message)
        return

    # Если же пользователь нажал inline-кнопку — она обрабатывается в callback'ах (send_order_inline)
    # и здесь мы не должны ничего делать. Просто возвращаем.
    return
...
def universal_handler(message):
    ...
    if data.get('wait_for_comment'):
        ...
        if text == t(chat_id, "send_order"):
            # Поддерживаем прежний путь через reply-текстовую кнопку, если она используется
            process_send_order(chat_id, message)
            return
