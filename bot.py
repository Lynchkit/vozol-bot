...
# ——— Настройки ———
TOKEN = os.getenv("TOKEN", "7931006644:AAFXpcKco4pqSsfCz05cW86bM7Ri7LVNL1g")
...
def get_flavors_keyboard(cat):
    ...
    for it in menu[cat]:
        if it.get("stock", 0) > 0:
            kb.add(f"{it['emoji']} {it['flavor']} ({it['price']}₺) [{it['stock']} шт]")
    kb.add("⬅️ Назад")
    return kb
...
def edit_action_keyboard():
    ...
    kb.add("💲 Fix Price")  # Новая кнопка для фиксации цены
    kb.add("❌ Cancel")
    return kb
...
def universal_handler(message):
    # Игнорируем команды, чтобы /start и /change работали отдельно
    ...
    if data.get('edit_phase'):
        ...
        if phase == 'add_category':
            ...
            data.pop('edit_cat', None)
            data['edit_phase'] = 'choose_action'
            bot.send_message(cid, f"Category «{new_cat}» added.", reply_markup=edit_action_keyboard())
            return
        ...
        if phase == 'add_flavor':
            ...
            data.pop('edit_cat', None)
            data['edit_phase'] = 'choose_action'
            bot.send_message(cid, f"Added «{name}» ({qty} pcs) to «{cat}».", reply_markup=edit_action_keyboard())
            return
                ...
                for it in menu[cat][:]:
                    if it['flavor'] == flavor_name:
                        if it['stock'] > 1:
                            ...
                            menu[cat].remove(it)
                        save_menu(menu)
                        bot.send_message(cid, f"Updated «{flavor_name}» in «{cat}».", reply_markup=edit_action_keyboard())
                        data.pop('edit_cat', None)
                        data['edit_phase'] = 'choose_action'
                        return
        ...
        if phase == 'choose_fix_price_cat':
            if text in menu:
                ...
                bot.send_message(cid, f"Enter new price in ₺ for all flavors in «{text}»:")
            else:
                bot.send_message(cid, "Choose category from the list.")
            return

        # 9) Ввод новой цены и обновление всех вкусов в категории
        if phase == 'enter_new_price':
            ...
            try:
                ...
                return
            # Обновляем цену у всех вкусов в выбранной категории
            for itm in menu.get(cat, []):
                itm['price'] = int(new_price)
            save_menu(menu)
            bot.send_message(cid, f"Price for all flavors in «{cat}» set to {int(new_price)}₺.", reply_markup=edit_action_keyboard())
            data.pop('edit_cat', None)
            data.pop('new_price', None)
            data['edit_phase'] = 'choose_action'
            return
    ...
    # — Если ожидаем ввод адреса — обработка локации/текста до блоков выбора категории —
    ...
    # — Если ожидаем ввод комментария — обработка перед блоками выбора —
    ...
    # — Обычный сценарий заказа (если мы не в стадии адрес/контакт/комментарий) —

    # «Назад» в меню выбора категории
    if text == "⬅️ Назад":
        data['current_category'] = None
    ...
    # Завершить заказ → запрос адреса (переходим в режим wait_for_address)
    ...
    # Выбор категории (если мы не в процессе адрес/контакт/комментарий)
    if text in menu:
        data['current_category'] = text
        bot.send_message(cid, f"Выберите вкус ({text}):", reply_markup=get_flavors_keyboard(text))
        return

    # Выбор вкуса (если у нас есть текущая категория)
    cat = data.get('current_category')
    if cat:
        for it in menu[cat]:
            label = f"{it['emoji']} {it['flavor']} ({it['price']}₺) [{it['stock']} шт]"
            if text == label and it.get('stock', 0) > 0:
                data['cart'].append({
                    'category': cat,
                    'emoji':    it['emoji'],
                    'flavor':   it['flavor'],
                ...
                bot.send_message(cid, f"{it['flavor']} добавлен в корзину.", reply_markup=kb)
                return

        # Если ввод не соответствует ни одному вкусу
