...
def handle_address_input(message):
    ...
    # Обработка кнопки «Назад» на шаге адреса
    if text == t(chat_id, "back"):
        # остаёмся на шаге ввода адреса, показываем клавиатуру заново
        data['wait_for_address'] = True
        bot.send_message(
            chat_id,
            t(chat_id, "enter_address"),  # ключ-подсказка: «Введите адрес»
            reply_markup=address_keyboard()  # клавиатура с кнопкой «Назад»
        )
        user_data[chat_id] = data
        return
